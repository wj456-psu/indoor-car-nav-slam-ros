...
# Developer: FuZhi, Liu (liu.fuzhi@wechangetech.com), Wisawawit Julathong (w.julathong@gmail.com)
#
...
class queue:  # class queue is design for uart receive data cache
...
class BaseControl:
    def __init__(self):
        ...
        self.ackermann_cmd_topic = rospy.get_param(
        ...
        try:
            ...
            try:
                ...
                rospy.loginfo("Opening Serial Try Failed")
                pass
        except:
        ...
        if ((base_type == 'NanoCar') & (self.sub_ackermann == True)):
            ...
            self.sub = rospy.Subscriber(
                self.ackermann_cmd_topic, AckermannDriveStamped, self.ackermannCmdCB, queue_size=20)
        else:
            self.sub = rospy.Subscriber(
                self.cmd_vel_topic, Twist, self.cmdCB, queue_size=20)
        self.pub = rospy.Publisher(self.odom_topic, Odometry, queue_size=10)
        self.battery_pub = rospy.Publisher(
            self.battery_topic, BatteryState, queue_size=3)
        self.tf_broadcaster = tf.TransformBroadcaster()
        self.timer_odom = rospy.Timer(rospy.Duration(
            1.0/self.odom_freq), self.timerOdomCB)
        self.timer_battery = rospy.Timer(rospy.Duration(
            1.0/self.battery_freq), self.timerBatteryCB)
        self.timer_communication = rospy.Timer(
            rospy.Duration(5.0/1000), self.timerCommunicationCB)
        # inorder to compatibility old version firmware,imu topic is NOT pud in default
        if (self.pub_imu):
            self.timer_imu = rospy.Timer(rospy.Duration(
                1.0/self.imu_freq), self.timerIMUCB)
    ...
    def crc_1byte(self, data: int):  # CRC-8 Calculate
        crc_1byte = 0
        for i in range(8):
    ...
    def cmdCB(self, data):  # Subscribe vel_cmd call this to send vel cmd to move base
        ...
        #     chr(0x00)
        output = [0x5a, 0x0c, 0x01, 0x01,
                  (int(self.trans_x*1000.0) >>
                   8) & 0xff, int(self.trans_x*1000.0) & 0xff,
                  (int(self.trans_y*1000.0) >>
                   8) & 0xff, int(self.trans_y*1000.0) & 0xff,
                  (int(self.rotat_z*1000.0) >>
                   8) & 0xff, int(self.rotat_z*1000.0) & 0xff,
                  0x00, 0x00
                  ]
        # outputdata = [0x5a, 0x0c, 0x01, 0x01, 0x00,
        ...
        crc_8 = self.crc_byte(output, len(output)-1)
        output[-1] = crc_8
        ...
        try:
            ...
            rospy.logerr("Vel Command Send Failed")
        self.serialIDLE_flag = 0
    ...
    def ackermannCmdCB(self, data):
        ...
        #     chr(0x00)
        output = [0x5a, 0x0c, 0x01, 0x15,
                  (int(self.speed*1000.0) >> 8) & 0xff, int(self.speed*1000.0) & 0xff,
                  (int(self.steering_angle*1000.0) >>
                   8) & 0xff, int(self.steering_angle*1000.0) & 0xff,
                  0x00, 0x00
                  ]
        # outputdata = [0x5a, 0x0c, 0x01, 0x15, 0x00,
        #               0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
            ...
            if len(reading) != 0:
                for i in range(0, len(reading)):
                    ...
                    try:
                        ...
                        # print(data)
                        # rospy.logerr("Circleloop.enqueue Failed")
                        pass
        else:
            pass
            ...
            if data == 0x5a:
                ...
                if length > 1:
                    if self.Circleloop.get_front_second() <= self.Circleloop.get_queue_length():
                        ...
                        if (databuf[-1]) == self.crc_byte(databuf, length-1):
                            pass
                        else:
                            # print(databuf)
                            # print("Crc check Err %d"%self.crc_byte(databuf,length-1))
                            pass
                        # rospy.loginfo(databuf)
                        # parse receive data
                        if (databuf[3] == 0x04):
                            # rospy.loginfo(
                            #     "Lower computer reports the current speed")
                            ...
                            # rospy.loginfo(
                            #     "Lower computer reports the IMU data")
                            self.Yawz = databuf[8]*256
                            self.Yawz += databuf[9]
                        elif (databuf[3] == 0x08):
                            # rospy.loginfo(
                            #     "Lower computer reports battery information")
                            ...
                            # rospy.loginfo(
                            #     "Lower computer reports the speed and heading information")
                            self.Vx = databuf[4]*256
                            self.Vx += databuf[5]
                            ...
                            # rospy.loginfo(
                            #     "Lower computer replies the version number, format is hardware version number xx.yy.zz, software version number aa.bb.cc")
                            ...
                            # rospy.loginfo(
                            #     "Lower computer reply version number, SN number")
                            sn_string = "SN:"
                            for i in range(4, 16):
                            ...
                            # rospy.loginfo(
                            #     "Data length of the configuration information returned by the lower computer")
    ...
    def getVersion(self):
        # Get version info
        # for python 2
        # output = chr(0x5a) + chr(0x06) + chr(0x01) + chr(0xf1) + \
        #     chr(0x00) + chr(0xd7)  # 0xd7 is CRC-8 value
        output = [0x5a, 0x06, 0x01, 0xf1, 0x00, 0xd7]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        ...
        try:
            ...
            rospy.logerr("Get Version Command Send Failed")
        self.serialIDLE_flag = 0
    ...
    def getSN(self):
        # Get version info
        # for python 2
        # output = chr(0x5a) + chr(0x06) + chr(0x01) + chr(0xf3) + \
        #     chr(0x00) + chr(0x46)  # 0x46 is CRC-8 value
        output = [0x5a, 0x06, 0x01, 0xf3, 0x00, 0x46]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 1
        try:
            ...
            rospy.logerr("Get SN Command Send Failed")
        self.serialIDLE_flag = 0
    ...
    def getInfo(self):
        # Get version info
        # for python 2
        # output = chr(0x5a) + chr(0x06) + chr(0x01) + chr(0x21) + \
        #     chr(0x00) + chr(0x8f)  # 0x8f is CRC-8 value
        output = [0x5a, 0x06, 0x01, 0x21, 0x00, 0x8f]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 1
        try:
            ...
            rospy.logerr("Get info Command Send Failed")
        self.serialIDLE_flag = 0
    ...
    def timerOdomCB(self, event):
        ...
        if self.movebase_firmware_version[1] == 0:
            ...
            # for python 2
            # output = chr(0x5a) + chr(0x06) + chr(0x01) + chr(0x09) + \
            #     chr(0x00) + chr(0x38)  # 0x38 is CRC-8 value
            output = [0x5a, 0x06, 0x01, 0x09, 0x00, 0x38]
        else:
            # in firmware version new than v1.1.0,support this command
            # for python 2
            # output = chr(0x5a) + chr(0x06) + chr(0x01) + \
            #     chr(0x11) + chr(0x00) + chr(0xa2)
            output = [0x5a, 0x06, 0x01, 0x11, 0x00, 0xa2]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 1
        try:
            ...
            rospy.logerr("Odom Command Send Failed")
        self.serialIDLE_flag = 0
        # calculate odom data
    ...
    def timerBatteryCB(self, event):
        # for python 2
        # output = chr(0x5a) + chr(0x06) + chr(0x01) + chr(0x07) + \
        #     chr(0x00) + chr(0xe4)  # 0xe4 is CRC-8 value
        output = [0x5a, 0x06, 0x01, 0x07, 0x00, 0xe4]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        ...
        try:
            ...
            rospy.logerr("Battery Command Send Failed")
        self.serialIDLE_flag = 0
        msg = BatteryState()
        msg.header.stamp = self.current_time
    ...
    def timerIMUCB(self, event):
        ...
        output = [0x5a, 0x06, 0x01, 0x13, 0x00, 0x33]
        while (self.serialIDLE_flag):
            time.sleep(0.01)
        self.serialIDLE_flag = 3
        try:
            ...
            rospy.logerr("Imu Command Send Failed")
        ...
        if (base_type == 'NanoRobot'):
            self.tf_broadcaster.sendTransform(
                (-0.062, 0.0235, 0.08), (0.0, 0.0, 0.0, 1.0), self.current_time, self.imuId, self.baseId)
        elif (base_type == 'NanoCar'):
            self.tf_broadcaster.sendTransform(
                (-0.056, 0.0285, 0.09), (0.0, 0.0, 0.0, 1.0), self.current_time, self.imuId, self.baseId)
        elif (base_type == '4WD'):
            self.tf_broadcaster.sendTransform(
...
if __name__ == "__main__":
    try:
        ...
        if base_type != None:
            rospy.loginfo(f"{base_type} base control ...")
        else:
