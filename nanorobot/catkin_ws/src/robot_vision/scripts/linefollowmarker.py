...
class LineFollowVisualization:
    ...
    def image_callback(self, data):
        ...
        # Publish visualization marker for the predicted line
        marker = Marker()
        marker.header.frame_id = data.header.frame_id
        marker.header.stamp = rospy.Time.now()
        marker.ns = "line_follow"
        marker.id = 0
        ...
        marker.points = []
        marker.points.append(data.header.frame_id)  # Start point
        marker.points.append(data.header.frame_id)  # End point
...
if __name__ == '__main__':
    try:
        rospy.init_node("line_follow_visualization")
        LineFollowVisualization()
        rospy.loginfo("Line follow visualization is started.")
        rospy.loginfo("Please subscribe to the ROS image.")
        rospy.spin()
    except KeyboardInterrupt:
        print("Shutting down line follow visualization node.")
        cv2.destroyAllWindows()
