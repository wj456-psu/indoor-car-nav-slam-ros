...
class CamShiftNode(ROS2OpenCV2):
    def __init__(self, node_name):
        ...
        cv.createTrackbar("Saturation", "Parameters",
                          self.smin, 255, self.set_smin)
        cv.createTrackbar("Min Value", "Parameters",
                          self.vmin, 255, self.set_vmin)
        cv.createTrackbar("Max Value", "Parameters",
                          self.vmax, 255, self.set_vmax)
        cv.createTrackbar("Threshold", "Parameters",
                          self.threshold, 255, self.set_threshold)
    ...
    def process_image(self, cv_image):
        try:
            ...
            mask = cv2.inRange(hsv, np.array(
                (0., self.smin, self.vmin)), np.array((180., 255., self.vmax)))
