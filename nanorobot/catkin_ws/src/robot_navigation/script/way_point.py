...
class MultiGoals:
    def __init__(self, goalListX, goalListY, goalListZ, loopTimes, map_frame):
        self.sub = rospy.Subscriber(
            'move_base/result', MoveBaseActionResult, self.statusCB, queue_size=10)
...
if __name__ == "__main__":
    try:
        ...
        goalListX = ast.literal_eval(goalListX)
        goalListY = ast.literal_eval(goalListY)
        goalListZ = ast.literal_eval(goalListZ)
        if len(goalListX) == len(goalListY) == len(goalListZ) & len(goalListY) >= 2:
            # Constract MultiGoals Obj
            rospy.loginfo("Multi Goals Executing...")
            mg = MultiGoals(goalListX, goalListY,
                            goalListZ, loopTimes, map_frame)
            rospy.spin()
        else:
